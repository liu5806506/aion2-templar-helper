...
# 检查并请求管理员权限（仅在需要时）
if sys.platform.startswith("win") and not os.environ.get('ADMIN_CHECKED'):
    try:
...
from modules.logic import LogicController
...
def check_dependencies():
    ...
    if missing_deps:
        print(f"缺少以下依赖库: {', '.join(missing_deps)}")
...
def main():
    ...
    try:
        ...
        # 初始化智能战斗逻辑
        combat_logic = LogicController(input_ctrl, None)  # Vision will be initialized inside LogicController
        global_logger.info("智能战斗逻辑初始化成功")
        ...
        # 测试键盘输入功能
        global_logger.info("测试键盘输入功能...")
        test_input_success = True
        try:
            # 测试输入控制器
            input_ctrl.press_key('a', 0.01, 0.02)
            global_logger.info("键盘输入测试成功")
        except Exception as e:
            global_logger.error(f"键盘输入测试失败: {e}")
            test_input_success = False
        
        if not test_input_success:
            global_logger.warning("键盘输入可能存在问题，某些游戏可能需要管理员权限或特殊设置才能接收输入")
